...
YuvConverter::YuvConverter() : formatPrinted(false), pktCount(0) {}

void YuvConverter::printPixelFormat(const AVFrame* frame) {
...
void YuvConverter::decodeVideo(AVCodecContext* codecCtx, AVPacket* pkt, AVFrame* frame, std::ofstream& ofs) {
  int ret;
  bool toReceive = true;
  ...
  while(toReceive) {
    ...
    // 以下是逐行写入的方法，这实际上更严谨，但是为了简单起见，就不这样写了
...
void YuvConverter::videoToYuv(VideoType type, const std::string_view& videoPath, const std::string_view& yuvPath) {
